...
var nomColonnesVerb = new Array ("MainValue", "Prefixe");
var nomColonnesAdj = new Array ("MainValue", "Feminin", "Prefixe");
var nomColonnesNoms = new Array ("MainValue","Genre", "Prefixe");
var nomColonnesPref = new Array ("MainValue");
var nomColonnesGab = new Array ("MainValue");
...
catch(e)
{
	...
	//MakeDisplay(); //with this everything is doubled in the display.
...
public function NewCorvoPhrase()
{
	...
	//Et ensuite on passe aux choses sérieuses, on cherche à remplacer les trucs un par un
...
function FindNom()
{
	...
	return [nomSelected,prefixeOrNot]; //Le nom ici n'a pas de déterminant !!
}
...
{
	Debug.Log("Let's build some DB UI");
	//On détruit l'affichage précédent
	...
	for(var element in donneesBase)
	{
		...
		textPrefab.name = count.ToString();
...
public function SupprimerDeLaBase()
{
	var count = 0;
	var tableToUse = FindTable();
	...
	for(var display in GameObject.FindGameObjectsWithTag("Display")){
		if(display.transform.GetChild(0).GetComponent(Image).enabled == true)
		{
			...
			{
				Debug.Log("trouvé un objet à supprimer !");
				texts.Add(display);
				count++;
			}
		}
