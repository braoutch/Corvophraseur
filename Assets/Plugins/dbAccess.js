...
/*  Javascript class for accessing SQLite objects.  
     ...
     //http://forum.unity3d.com/threads/sqlite-class-easier-database-stuff.28500/
...
class dbAccess {
    ...
    //connection = "URI=file:" + Application.persistentDataPath + "/" + p; // the solution on android
    ...
    function DeleteBase(baseName : String){
    	var query : String;
    	query = "DROP DATABASE " + baseName;
    	dbcmd = dbcon.CreateCommand();
    	dbcmd.CommandText = query; 
    	reader = dbcmd.ExecuteReader();
    }

    function DeleteFrom (tableName : String,itemToDelete : String, wPar : String,wValue : String)
    {
    	var query : String;
    ...
    function UpdateFrom(tableName : String,itemToSet : String, wPar : String,wValue : String, itemToFind : String, wParToFind : String, wValueToFind : String)
    {
    	var query : String;
    	query = "UPDATE" + tableName + " SET " + itemToSet + wPar + wValue + " WHERE " + itemToFind + wParToFind + wValueToFind ;    
    	dbcmd = dbcon.CreateCommand();
    	dbcmd.CommandText = query;
    	reader = dbcmd.ExecuteReader();
    }  

    function SingleSelectWhereFloat(tableName : String, itemToSelect : String, wCol : String, wPar : String, wValue : String):float
    { 
    	var query : String;
        ...
        var readArray:List.<float> = new List.<float>();
        while(reader.Read()) 
        { 
            //readArray.Push(reader.GetString(0)); // Fill array with all matches
            var value:float = reader.GetFloat(0);
            //Debug.Log(value);
        return value; // return matches
    }
}
...
{ 
	...
	reader = dbcmd.ExecuteReader();
        ...
        { 
            //readArray.Push(reader.GetString(0)); // Fill array with all matches
            var value:String = reader.GetString(0);
            //Debug.Log(value);
        return value; // return matches
    }
}

 function ReadFullTableOrdered(tableName : String, orderingColumn : String) {
        var query : String;
        query             = "SELECT * FROM " + tableName + " ORDER BY " + orderingColumn + " DESC";
        dbcmd             = dbcon.CreateCommand();
        dbcmd.CommandText = query; 
        reader            = dbcmd.ExecuteReader();
        var readArray = new ArrayList();
 ...
 //////////////////////////PLUS LES MIENNES A PARTIR D'ICI
